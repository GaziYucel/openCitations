...
class Deposit
{
    ...
    public function execute(): void
    {
        if (
            ...
            !empty($this->publication->getData(Constants::depositedUrlName)) ||
            empty($this->publicationDoi)
        ) {
            return;
        }
        ...
        if ($githubIssueId) {
            ...
            $this->publication->setData(
                ...
                Constants::githubUrl . "/$this->owner/$this->repository/issues/$githubIssueId"
            );

            //fixme: without this line, publication doesn't gets saved when job run with delay
            error_log(Constants::githubUrl . "/$this->owner/$this->repository/issues/$githubIssueId");
    ...
    private function addIssue(string $title, string $body): string
    {
        try {
            ...
            $response = $client->request(
                'POST',
                Constants::apiUrl . "/$this->owner/$this->repository/issues",
                [
                    'json' =>
                        [
                            'title' => $title,
